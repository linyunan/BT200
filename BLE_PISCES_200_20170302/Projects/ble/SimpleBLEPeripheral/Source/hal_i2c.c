 ...
 * input parameters
 *
 * @param       addr - address of  24c64 register. 
 * @param       len - Number of bytes to read.
 * @param       pBuf - Pointer to the data buffer to put read bytes.
 *
 * output parameters
...
uint16 AT24C64_I2C_Read(uint16 addr, uint16 len, void  *pBuf)//aa[7])
{
  uint8 addrL = 0, addrH = 0, addrHH = 0; 
  uint8 cnt;
  static uint8 * p;
  p = pBuf;
  HalI2CInit(E2P_24C64_ADDR, i2cClock_123KHZ);
  
  addrL = (uint8)(addr & 0xff); 
  addrH = (uint8)(addr >> 8);   
  addrHH = (uint8)( i2cAddr ); // i2cAddr 器件地址
  
  //Dummy Write Start_Signal
  I2C_STRT();

  //Dummy Write Device_Addr.
  if ( I2CSTAT == mstStarted ) // A start condition has been transmitted
  {
    I2C_WRITE( addrHH & 0xFE ); //set LSB = 0 for Write
  }
  ...
  I2C_STOP();
  //Write Start_Signal for Read
  I2C_STRT();

  if ( I2CSTAT == mstStarted ) // A start condition has been transmitted
  {
    I2C_WRITE( addrHH | 0x01 ); //set LSB = 1 for Read
  }
...
uint16 AT24C64_I2C_Write(uint16 addr, uint16 len, void *pBuf)
{
  uint8 addrL = 0, addrH = 0, addrHH = 0;   
  uint32 wlen = 0;
  uint16 j = 0;
...
ScrollPage:
  ...
  while((I2CSTAT != mstStarted) && j<10000) j++; 
  if(j>=10000)
  {
      I2C_STOP();
      return 0;
  ...
  while( (I2CSTAT != mstAddrAckW) && j<10000) j++;
  if(j>=10000)
  {
      I2C_STOP();
      return 0;
  ...
  while( (I2CSTAT != mstDataAckW) && j<10000) j++;
  if(j>=10000)
  {
      I2C_STOP();
      return 0;
